...
build_extension() {
  ...
  pnpm -w vsix -- --out ../bin/cybrosys-assista-$(git rev-parse --short HEAD).vsix || exit 1
  code --install-extension ../../bin/cybrosys-assista-$(git rev-parse --short HEAD).vsix || exit 1
  cd evals
}
...
if [[ "$(uname -s)" != "Darwin" ]]; then
  ...
  echo "See https://github.com/CybrosysAssistaInc/Cybrosys-Assista/blob/main/packages/evals/README.md for instructions."
  exit 1
fi
...
if [[ ! -d "../../../evals" ]]; then
  ...
  git clone https://github.com/CybrosysAssistaInc/Cybrosys-Assista-Evals.git ../../../evals || exit 1
  echo "✅ Done"
else
...
pnpm --filter @cybrosys-assista/evals db:push --force &>/dev/null || exit 1
echo "✅ Done"
...
current_version=$(code --list-extensions --show-versions 2>/dev/null | grep assista)
...
if ! nc -z localhost 3000; then
  ...
  if [[ "$start_evals" =~ ^[Yy]|^$ ]]; then
    pnpm --filter @cybrosys-assista/web-evals dev
  else
    echo "💡 You can start it anytime with 'pnpm --filter @cybrosys-assista/web-evals dev'."
  fi
else
