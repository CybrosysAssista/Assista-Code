import { AssistaMessage, HistoryItem } from "@cybrosys-assista/types"
import { useCallback, useEffect, useMemo, useState } from "react"

interface UsePromptHistoryProps {
	assistaMessages: AssistaMessage[] | undefined
...
export const usePromptHistory = ({
	...
	const filteredPromptHistory = useMemo(() => {
		// First try to get conversation messages (user_feedback from assistaMessages)
		const conversationPrompts = assistaMessages
		...
		// If we have assistaMessages array (meaning we're in an active task), don't fall back to task history
		// Only use task history when starting fresh (no active conversation)
		if (assistaMessages?.length) {
			return []
		}
	...
	}, [assistaMessages, taskHistory, cwd])
