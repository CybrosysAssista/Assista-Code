...
export async function createRun({ suite, exercises = [], systemPrompt, ...values }: CreateRun) {
	...
	try {
		...
		const cliCommand = `pnpm --filter @cybrosys-assista/evals cli --runId ${run.id}`
		...
		const logStream = fs.createWriteStream("/tmp/cybrosys-assista-evals.log", { flags: "a" })
