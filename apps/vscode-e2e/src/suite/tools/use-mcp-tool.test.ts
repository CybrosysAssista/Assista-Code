...
suite("Cybrosys Assista use_mcp_tool Tool", function () {
	...
	suiteSetup(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "assista-test-mcp-"))
		...
		testFiles = {
			...
			mcpConfig: path.join(workspaceDir, ".assista", "mcp.json"),
		}
		...
		// Create .assista directory and MCP configuration file
		const assistaDir = path.join(workspaceDir, ".assista")
	...
	suiteTeardown(async () => {
		...
		// Clean up .assista directory
		const workspaceDir = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath || tempDir
		const assistaDir = path.join(workspaceDir, ".assista")
		try {
			await fs.rm(assistaDir, { recursive: true, force: true })
		} catch {
	...
	test("Should request MCP filesystem read_file tool and complete successfully", async function () {
		...
		// Listen for messages
		const messageHandler = ({ message }: { message: AssistaMessage }) => {
		...
		await sleep(2000) // Wait for Cybrosys Assista to fully initialize
		...
		try {
			...
			// Make a small modification to trigger the save event, without this Cybrosys Assista won't load the MCP server
	...
	test("Should request MCP filesystem list_directory tool and complete successfully", async function () {
		...
		try {
			...
			const hasAssistaDir = responseText.includes(".assista")

			// At least one of our test files or the .assista directory should be present
			assert.ok(
				hasTestFile || hasDataFile || hasAssistaDir,
				`MCP server response should contain our test files or .assista directory. Expected to find: '${path.basename(testFiles.simple)}', '${path.basename(testFiles.testData)}', or '.assista'. Got: ${responseText.substring(0, 200)}...`,
			)
	...
	test.skip("Should validate MCP request message format and complete successfully", async function () {
		...
		const messageHandler = ({ message }: { message: AssistaMessage }) => {
			...
			if (message.type === "ask" && message.ask === "use_mcp_server") {
				...
				// Validate the message format matches AssistaAskUseMcpServer interface
				if (message.text) {
					try {
						const mcpRequest = JSON.parse(message.text)
						mcpToolName = mcpRequest.toolName
