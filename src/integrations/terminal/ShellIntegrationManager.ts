...
export class ShellIntegrationManager {
	...
	public static zshInitTmpDir(env: Record<string, string>): string {
		...
		const tmpDir = path.join(os.tmpdir(), `assista-zdotdir-${Math.random().toString(36).substring(2, 15)}`)
		console.info(`[TerminalRegistry] Creating temporary directory for ZDOTDIR: ${tmpDir}`)

		// Save original ZDOTDIR as ASSISTA_ZDOTDIR
		if (process.env.ZDOTDIR) {
			env.ASSISTA_ZDOTDIR = process.env.ZDOTDIR
		}
	...
	ZDOTDIR=\${ASSISTA_ZDOTDIR:-$HOME}
	unset ASSISTA_ZDOTDIR
