...
export class McpHub {
	...
	/**
	 * Registers a client (e.g., AssistaProvider) using this hub.
	...
	private async watchProjectMcpFile(): Promise<void> {
		...
		const projectMcpPattern = new vscode.RelativePattern(workspaceFolder, ".assista/mcp.json")
  ...
  "mcpServers": {
	...
	private async getProjectMcpPath(): Promise<string | null> {
		...
		const workspaceFolder = vscode.workspace.workspaceFolders[0]
		const projectMcpDir = path.join(workspaceFolder.uri.fsPath, ".assista")
	...
	private async notifyWebviewOfServerChanges(): Promise<void> {
		...
		const targetProvider: AssistaProvider | undefined = this.providerRef.deref()
