...
import type { AssistaProvider } from "../../../core/webview/AssistaProvider"
...
describe("McpHub", () => {
	...
	let mockProvider: Partial<AssistaProvider>
	...
	beforeEach(() => {
		...
		mcpHub = new McpHub(mockProvider as AssistaProvider)
	})
	...
	describe("toggleToolAlwaysAllow", () => {
		...
		it("should initialize alwaysAllow if it does not exist", async () => {
			...
			const normalizedSettingsPath = "/mock/settings/path/assista_mcp_settings.json"
			const writeCalls = vi.mocked(fs.writeFile).mock.calls
	...
	describe("Windows command wrapping", () => {
		...
		it("should wrap commands with cmd.exe on Windows", async () => {
			...
			const mcpHub = new McpHub(mockProvider as AssistaProvider)
