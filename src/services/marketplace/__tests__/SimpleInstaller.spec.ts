...
describe("SimpleInstaller", () => {
	...
	describe("installMode", () => {
		...
		it("should install mode when .assistamodes file does not exist", async () => {
			...
			expect(result.filePath).toBe(path.join("/test/workspace", ".assistamodes"))
		...
		it("should install mode when .assistamodes contains valid YAML", async () => {
		...
		it("should throw error when .assistamodes contains invalid YAML", async () => {
			...
			await expect(installer.installItem(mockModeItem, { target: "project" })).rejects.toThrow(
				"Cannot install mode: The .assistamodes file contains invalid YAML",
			)
	...
	describe("installMcp", () => {
		...
		it("should install MCP when mcp.json file does not exist", async () => {
			...
			expect(result.filePath).toBe(path.join("/test/workspace", ".assista", "mcp.json"))
			expect(mockFs.writeFile).toHaveBeenCalled()
		...
		it("should throw error when mcp.json contains invalid JSON", async () => {
			...
			await expect(installer.installItem(mockMcpItem, { target: "project" })).rejects.toThrow(
				"Cannot install MCP server: The .assista/mcp.json file contains invalid JSON",
			)
	...
	describe("removeMode", () => {
		...
		it("should throw error when .assistamodes contains invalid YAML during removal", async () => {
			const invalidYaml = "invalid: yaml: content: {"

			mockFs.readFile.mockResolvedValueOnce(invalidYaml)

			await expect(installer.removeItem(mockModeItem, { target: "project" })).rejects.toThrow(
				"Cannot remove mode: The .assistamodes file contains invalid YAML",
			)
