...
vi.mock("@cybrosys-assista/cloud", () => ({
	CloudService: {
		hasInstance: vi.fn(),
		instance: {
...
vi.mock("../../../shared/package", () => ({
	Package: {
		publisher: "cybrosys-assista",
		name: "cybrosys-assista",
		version: "1.0.0",
		outputChannel: "Cybrosys-Assista",
		sha: undefined,
	},
}))
...
import { CloudService, getClerkBaseUrl, PRODUCTION_CLERK_BASE_URL } from "@cybrosys-assista/cloud"
...
describe("MdmService", () => {
	...
	describe("platform-specific config paths", () => {
		...
		it("should use correct path for Linux in production", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/etc/cybrosys-assista/mdm.json")
		})
		...
		it("should use correct path for Linux in development", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/etc/cybrosys-assista/mdm.dev.json")
		})
	...
	describe("cloud enablement", () => {
		it("should enable Cybrosys Assista Cloud when MDM config is present and setting is disabled", async () => {
			...
			expect(mockVscode.workspace.getConfiguration).toHaveBeenCalledWith("cybrosys-assista")
