...
import type { GlobalState, ProviderSettings, ModelInfo } from "@cybrosys-assista/types"
...
describe("Assista", () => {
	...
	beforeEach(() => {
		...
		mockProvider = new AssistaProvider(
			mockExtensionContext,
			mockOutputChannel,
			"sidebar",
	...
	describe("constructor", () => {
		it("should respect provided settings", async () => {
			...
			expect(assista.diffEnabled).toBe(false)
		})
		...
		it("should use default fuzzy match threshold when not provided", async () => {
			...
			expect(assista.diffEnabled).toBe(true)

			// The diff strategy should be created with default threshold (1.0).
			expect(assista.diffStrategy).toBeDefined()
		})
	...
	describe("getEnvironmentDetails", () => {
		describe("API conversation handling", () => {
			it.skip("should clean conversation history before sending to API", async () => {
				// Assista.create will now use our mocked getEnvironmentDetails
				const [assista, task] = Task.create({
					provider: mockProvider,
					apiConfiguration: mockApiConfig,
					task: "test task",
				})

				assista.abandoned = true
				await task
				...
				vi.spyOn(assista.api, "createMessage").mockImplementation(cleanMessageSpy)

				// Add test message to conversation history.
				assista.apiConversationHistory = [
					{
						role: "user" as const,
				...
				// Mock abort state
				Object.defineProperty(assista, "abort", {
					get: () => false,
					set: () => {},
					configurable: true,
				})
				...
				assista.apiConversationHistory = [messageWithExtra]

				// Trigger an API request
				await assista.recursivelyMakeAssistaRequests([{ type: "text", text: "test request" }], false)
			...
			it.skip("should handle image blocks based on model capabilities", async () => {
				...
				const [assistaWithImages, taskWithImages] = Task.create({
					provider: mockProvider,
				...
				vi.spyOn(assistaWithImages.api, "getModel").mockReturnValue({
					id: "claude-3-sonnet",
					info: {
						supportsImages: true,
				...
				assistaWithImages.apiConversationHistory = conversationHistory

				// Test with model that doesn't support images
				const [assistaWithoutImages, taskWithoutImages] = Task.create({
					provider: mockProvider,
				...
				vi.spyOn(assistaWithoutImages.api, "getModel").mockReturnValue({
					id: "gpt-3.5-turbo",
					info: {
				...
				assistaWithoutImages.apiConversationHistory = conversationHistory

				// Mock abort state for both instances
				Object.defineProperty(assistaWithImages, "abort", {
					get: () => false,
					set: () => {},
					configurable: true,
				})

				Object.defineProperty(assistaWithoutImages, "abort", {
					get: () => false,
					set: () => {},
					configurable: true,
				})
				...
				vi.spyOn(assistaWithImages.api, "createMessage").mockImplementation(imagesSpy)
				vi.spyOn(assistaWithoutImages.api, "createMessage").mockImplementation(noImagesSpy)

				// Set up conversation history with images
				assistaWithImages.apiConversationHistory = [
					{
						role: "user",
						content: [
				...
				assistaWithImages.abandoned = true
				await taskWithImages.catch(() => {})

				assistaWithoutImages.abandoned = true
				await taskWithoutImages.catch(() => {})

				// Trigger API requests
				await assistaWithImages.recursivelyMakeAssistaRequests([{ type: "text", text: "test request" }])
				await assistaWithoutImages.recursivelyMakeAssistaRequests([{ type: "text", text: "test request" }])
			...
			it.skip("should handle API retry with countdown", async () => {
				...
				const saySpy = vi.spyOn(assista, "say")
				...
				vi.spyOn(assista.api, "createMessage").mockImplementation(() => {
					if (firstAttempt) {
						firstAttempt = false
				...
				// Trigger API request
				const iterator = assista.attemptApiRequest(0)
				await iterator.next()
			...
			describe("processUserContentMentions", () => {
				it("should process mentions in task and feedback tags", async () => {
					...
					const processedContent = await processUserContentMentions({
						userContent,
						cwd: assista.cwd,
						urlContentFetcher: assista.urlContentFetcher,
						fileContextTracker: assista.fileContextTracker,
					})
