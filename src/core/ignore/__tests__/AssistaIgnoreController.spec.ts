// npx vitest core/ignore/__tests__/AssistaIgnoreController.spec.ts

import type { Mock } from "vitest"

import { AssistaIgnoreController, LOCK_TEXT_SYMBOL } from "../AssistaIgnoreController"
...
describe("AssistaIgnoreController", () => {
	const TEST_CWD = "/test/path"
	let controller: AssistaIgnoreController
	...
	beforeEach(() => {
		...
		// Create controller
		controller = new AssistaIgnoreController(TEST_CWD)
	})

	describe("initialization", () => {
		/**
		 * Tests the controller initialization when .assistaignore exists
		 */
		it("should load .assistaignore patterns on initialization when file exists", async () => {
			// Setup mocks to simulate existing .assistaignore file
			...
			expect(mockFileExists).toHaveBeenCalledWith(path.join(TEST_CWD, ".assistaignore"))
			expect(mockReadFile).toHaveBeenCalledWith(path.join(TEST_CWD, ".assistaignore"), "utf8")

			// Verify content was stored
			expect(controller.assistaIgnoreContent).toBe("node_modules\n.git\nsecrets.json")
		...
		/**
		 * Tests the controller behavior when .assistaignore doesn't exist
		 */
		it("should allow all access when .assistaignore doesn't exist", async () => {
			// Setup mocks to simulate missing .assistaignore file
			...
			expect(controller.assistaIgnoreContent).toBeUndefined()
		 ...
		 */
		it("should set up file watcher for .assistaignore changes", async () => {
			// Check that watcher was created with correct pattern
			expect(vscode.workspace.createFileSystemWatcher).toHaveBeenCalledWith(
				expect.objectContaining({
					base: TEST_CWD,
					pattern: ".assistaignore",
				}),
			)
		 ...
		 */
		it("should handle errors when loading .assistaignore", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith("Unexpected error loading .assistaignore:", expect.any(Error))
	...
	describe("validateAccess", () => {
		...
		/**
		 * Tests the default behavior when no .assistaignore exists
		 */
		it("should allow all access when no .assistaignore content", async () => {
			// Create a new controller with no .assistaignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new AssistaIgnoreController(TEST_CWD)
	...
	describe("validateCommand", () => {
		...
		/**
		 * Tests behavior when no .assistaignore exists
		 */
		it("should allow all commands when no .assistaignore exists", async () => {
			// Create a new controller with no .assistaignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new AssistaIgnoreController(TEST_CWD)
			await emptyController.initialize()
	...
	describe("getInstructions", () => {
		/**
		 * Tests instructions generation with .assistaignore
		 */
		it("should generate formatted instructions when .assistaignore exists", async () => {
			// Setup .assistaignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**")
			await controller.initialize()
		 ...
		 * Tests behavior when no .assistaignore exists
		 */
		it("should return undefined when no .assistaignore exists", async () => {
			// Setup no .assistaignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
	...
	describe("file watcher", () => {
		/**
		 * Tests behavior when .assistaignore is created
		 */
		it("should reload .assistaignore when file is created", async () => {
			// Setup initial state without .assistaignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
			...
			// Create and initialize controller with no .assistaignore
			controller = new AssistaIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Force reload of .assistaignore content manually
			await controller.initialize()
		...
		/**
		 * Tests behavior when .assistaignore is changed
		 */
		it("should reload .assistaignore when file is changed", async () => {
			// Setup initial state with .assistaignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
		...
		/**
		 * Tests behavior when .assistaignore is deleted
		 */
		it("should reset when .assistaignore is deleted", async () => {
			// Setup initial state with .assistaignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
