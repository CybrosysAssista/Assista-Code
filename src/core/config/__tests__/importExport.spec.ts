...
import type { ProviderName } from "@cybrosys-assista/types"
	...
	describe("exportSettings", () => {
		...
		it("should export settings to the selected file location", async () => {
			;(vscode.window.showSaveDialog as Mock).mockResolvedValue({
				fsPath: "/mock/path/cybrosys-assista-settings.json",
			})
			...
			expect(fs.writeFile).toHaveBeenCalledWith(
				"/mock/path/cybrosys-assista-settings.json",
		...
		it("should use the correct default save location", async () => {
			...
			expect(vscode.Uri.file).toHaveBeenCalledWith(path.join("/mock/home", "Documents", "cybrosys-assista-settings.json"))
		})
	})
})
