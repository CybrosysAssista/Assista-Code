...
describe("CustomModesManager", () => {
	...
	const mockAssistamodes = `${path.sep}mock${path.sep}workspace${path.sep}.assistamodes`
	...
	beforeEach(() => {
		...
		;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
			return path === mockSettingsPath || path === mockAssistamodes
		})
	...
	describe("getCustomModes", () => {
		it("should handle valid YAML in .assistamodes file and JSON for global customModes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]

			const assistamodesModes = [{ slug: "mode2", name: "Mode 2", roleDefinition: "Role 2", groups: ["read"] }]

			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockSettingsPath) {
					return yaml.stringify({ customModes: settingsModes })
				}
				if (path === mockAssistamodes) {
					return yaml.stringify({ customModes: assistamodesModes })
				}
				throw new Error("File not found")
			})
		...
		it("should merge modes with .assistamodes taking precedence", async () => {
			...
			// Should contain 3 modes (mode1 from settings, mode2 and mode3 from assistamodes)
			expect(modes).toHaveLength(3)
			expect(modes.map((m) => m.slug)).toEqual(["mode2", "mode3", "mode1"])

			// mode2 should come from .assistamodes since it takes precedence
		...
		it("should handle missing .assistamodes file", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
		...
		it("should handle invalid YAML in .assistamodes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			...
			// Should fall back to settings modes when .assistamodes is invalid
			expect(modes).toHaveLength(1)
			expect(modes[0].slug).toBe("mode1")
		})
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .assistamodes precedence", async () => {
			...
			let assistamodesContent = { customModes: assistamodesModes }

			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockAssistamodes) {
					return yaml.stringify(assistamodesContent)
				}
				if (path === mockSettingsPath) {
			...
			;(fs.writeFile as Mock).mockImplementation(async (path: string, content: string, _encoding?: string) => {
				...
				if (path === mockAssistamodes) {
					assistamodesContent = yaml.parse(content)
				}
			...
			// Should update global state with merged modes where .assistamodes takes precedence
			expect(mockContext.globalState.update).toHaveBeenCalledWith(
				"customModes",
				expect.arrayContaining([
					expect.objectContaining({
						slug: "mode1",
						name: "Assistamodes Mode 1", // .assistamodes version should take precedence
						source: "project",
					}),
				]),
			)
		...
		it("creates .assistamodes file when adding project-specific mode", async () => {
			...
			// Mock .assistamodes to not exist initially
			let assistamodesContent: any = null
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
			...
			// Verify .assistamodes was created with the project mode
			...
			const writeCall = (fs.writeFile as Mock).mock.calls[0]
			expect(path.normalize(writeCall[0])).toBe(path.normalize(mockAssistamodes))

			// Verify the content written to .assistamodes
			expect(assistamodesContent).toEqual({
				customModes: [
					expect.objectContaining({
						slug: "project-mode",
						name: "Project Mode",
