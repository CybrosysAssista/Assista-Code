...
describe("getEnvironmentDetails", () => {
	...
	beforeEach(() => {
		...
		mockAssista = {
			...
			assistaIgnoreController: {
				...
				dispose: vi.fn(),
			} as unknown as AssistaIgnoreController,
			assistaMessages: [],
			api: {
			...
			} as unknown as WeakRef<AssistaProvider>,
		}
	...
	it("should return basic environment details", async () => {
		const result = await getEnvironmentDetails(mockAssista as Task)
		...
		expect(getApiMetrics).toHaveBeenCalledWith(mockAssista.assistaMessages)
	})

	it("should include file details when includeFileDetails is true", async () => {
		const result = await getEnvironmentDetails(mockAssista as Task, true)
	...
	it("should not include file details when includeFileDetails is false", async () => {
		await getEnvironmentDetails(mockAssista as Task, false)
	...
	it("should include recently modified files if any", async () => {
		;(mockAssista.fileContextTracker!.getAndClearRecentlyModifiedFiles as Mock).mockReturnValue([
			"modified1.ts",
			"modified2.ts",
		])
	...
	it("should include active terminal information", async () => {
		...
		mockAssista.didEditFile = true
		await getEnvironmentDetails(mockAssista as Task)
	...
	it("should handle missing provider or state", async () => {
		...
		mockAssista.providerRef!.deref = vi.fn().mockReturnValue(null)
		...
		mockAssista.providerRef!.deref = vi.fn().mockReturnValue({
			getState: vi.fn().mockResolvedValue(null),
		})

		const result2 = await getEnvironmentDetails(mockAssista as Task)
	...
	it("should handle errors gracefully", async () => {
		...
		await expect(getEnvironmentDetails(mockAssista as Task)).resolves.not.toThrow()
	})
})
