// npx vitest core/prompts/__tests__/responses-assistaignore.spec.ts
...
describe("AssistaIgnore Response Formatting", () => {
	...
	describe("formatResponse.assistaIgnoreError", () => {
		/**
		 * Tests the error message format for ignored files
		 */
		it("should format error message for ignored files", () => {
			const errorMessage = formatResponse.assistaIgnoreError("secrets/api-keys.json")

			// Verify error message format
			expect(errorMessage).toContain("Access to secrets/api-keys.json is blocked by the .assistaignore file settings")
			expect(errorMessage).toContain("continue in the task without using this file")
			expect(errorMessage).toContain("ask the user to update the .assistaignore file")
		})
		...
		it("should include the file path in the error message", () => {
			...
			for (const testPath of paths) {
				const errorMessage = formatResponse.assistaIgnoreError(testPath)
				expect(errorMessage).toContain(`Access to ${testPath} is blocked`)
			}
		})
	})

	describe("formatResponse.formatFilesList with AssistaIgnoreController", () => {
		/**
		 * Tests file listing with assistaignore controller
		 */
		it("should format files list with lock symbols for ignored files", async () => {
			// Create controller
			const controller = new AssistaIgnoreController(TEST_CWD)
		...
		/**
		 * Tests formatFilesList when showAssistaIgnoredFiles is set to false
		 */
		it("should hide ignored files when showAssistaIgnoredFiles is false", async () => {
			// Create controller
			const controller = new AssistaIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Format with controller and showAssistaIgnoredFiles = false
			const result = formatResponse.formatFilesList(
				TEST_CWD,
				files,
				false,
				controller as any,
				false, // showAssistaIgnoredFiles = false
			)
		...
		/**
		 * Tests formatFilesList handles truncation correctly with AssistaIgnoreController
		 */
		it("should handle truncation with AssistaIgnoreController", async () => {
			// Create controller
			const controller = new AssistaIgnoreController(TEST_CWD)
			await controller.initialize()
		 ...
		 */
		it("should handle empty file list with AssistaIgnoreController", async () => {
			// Create controller
			const controller = new AssistaIgnoreController(TEST_CWD)
			await controller.initialize()
	...
	describe("getInstructions", () => {
		 ...
		 */
		it("should format .assistaignore instructions for the LLM", async () => {
			// Create controller
			const controller = new AssistaIgnoreController(TEST_CWD)
			await controller.initialize()
		...
		it("should return undefined when no .assistaignore exists", async () => {
			// Set up no .assistaignore
			mockFileExists.mockResolvedValue(false)

			// Create controller without .assistaignore
			const controller = new AssistaIgnoreController(TEST_CWD)
			await controller.initialize()
