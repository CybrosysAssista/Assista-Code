...
import { ModeConfig } from "@cybrosys-assista/types"
...
__setMockImplementation(
	async (
		...
		const rules = []
		if (mode) {
			rules.push(`# Rules from .assistarules-${mode}:\nMock mode-specific rules`)
		}
		rules.push(`# Rules from .assistarules:\nMock generic rules`)
...
describe("SYSTEM_PROMPT", () => {
	...
	it("should maintain consistent system prompt", async () => {
		const prompt = await SYSTEM_PROMPT(
			...
			undefined, // language
			undefined, // assistaIgnoreInstructions
