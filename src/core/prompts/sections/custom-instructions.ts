...
import { isLanguage } from "@cybrosys-assista/types"
...
export async function loadRuleFiles(cwd: string): Promise<string> {
	// Check for .assista/rules/ directory
	const assistaRulesDir = path.join(cwd, ".assista", "rules")
	if (await directoryExists(assistaRulesDir)) {
		const files = await readTextFilesFromDirectory(assistaRulesDir)
		if (files.length > 0) {
			return formatDirectoryContent(assistaRulesDir, files)
		}
	}

	// Fall back to existing behavior
	const ruleFiles = [".assistarules", ".assistarules"]
...
export async function addCustomInstructions(
	...
	cwd: string,
	mode: string,
	options: { language?: string; assistaIgnoreInstructions?: string } = {},
): Promise<string> {
	const sections = []
	...
	if (mode) {
		// Check for .assista/rules-${mode}/ directory
		const modeRulesDir = path.join(cwd, ".assista", `rules-${mode}`)
		...
		if (!modeRuleContent) {
			const assistaModeRuleFile = `.assistarules-${mode}`
			modeRuleContent = await safeReadFile(path.join(cwd, assistaModeRuleFile))
			if (modeRuleContent) {
				usedRuleFile = assistaModeRuleFile
			} else {
				const assistaModeRuleFile = `.assistarules-${mode}`
				modeRuleContent = await safeReadFile(path.join(cwd, assistaModeRuleFile))
				if (modeRuleContent) {
	...
	if (modeRuleContent && modeRuleContent.trim()) {
		if (usedRuleFile.includes(path.join(".assista", `rules-${mode}`))) {
	...
	if (options.assistaIgnoreInstructions) {
		rules.push(options.assistaIgnoreInstructions)
	}
