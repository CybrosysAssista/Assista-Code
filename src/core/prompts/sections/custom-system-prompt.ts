...
export function getSystemPromptFilePath(cwd: string, mode: Mode): string {
	return path.join(cwd, ".assista", `system-prompt-${mode}`)
}

/**
 * Loads custom system prompt from a file at .assista/system-prompt-[mode slug]
...
/**
 * Ensures the .assista directory exists, creating it if necessary
 */
export async function ensureAssistaDirectory(cwd: string): Promise<void> {
	const assistaDir = path.join(cwd, ".assista")

	// Check if directory already exists
	if (await fileExistsAtPath(assistaDir)) {
		return
	}
