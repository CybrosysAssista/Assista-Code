// npx vitest core/webview/__tests__/AssistaProvider.spec.ts

import Anthropic from "@anthropic-ai/sdk"
import * as vscode from "vscode"
import axios from "axios"

import { type ProviderSettingsEntry, type AssistaMessage, ORGANIZATION_ALLOW_ALL } from "@cybrosys-assista/types"
	...
	Task: vi
		...
		.mockImplementation(
			(_provider, _apiConfiguration, _customInstructions, _diffEnabled, _fuzzyMatchThreshold, _task, taskId) => ({
				...
				overwriteAssistaMessages: vi.fn(),
...
describe("AssistaProvider", () => {
	...
	beforeEach(() => {
		...
		provider = new AssistaProvider(mockContext, mockOutputChannel, "sidebar", new ContextProxy(mockContext))
	...
	test("constructor initializes correctly", () => {
		expect(provider).toBeInstanceOf(AssistaProvider)
		// Since getVisibleInstance returns the last instance where view.visible is true
		// @ts-ignore - accessing private property for testing
		provider.view = mockWebviewView
		expect(AssistaProvider.getVisibleInstance()).toBe(provider)
	})
	...
	test("clearTask aborts current task", async () => {
		// Setup Assista instance with auto-mock from the top of the file
		const mockAssista = new Task(defaultTaskOptions) // Create a new mocked instance

		// add the mock object to the stack
		await provider.addAssistaToStack(mockAssista)

		// get the stack size before the abort call
		const stackSizeBeforeAbort = provider.getAssistaStackSize()

		// call the removeAssistaFromStack method so it will call the current assista abort and remove it from the stack
		await provider.removeAssistaFromStack()

		// get the stack size after the abort call
		const stackSizeAfterAbort = provider.getAssistaStackSize()

		// check if the abort method was called
		expect(mockAssista.abortTask).toHaveBeenCalled()
	...
	describe("clearTask message handler", () => {
		...
		test("calls clearTask when there is no parent task", async () => {
			...
			const mockAssista = new Task(defaultTaskOptions)
		...
		test("calls finishSubTask when there is a parent task", async () => {
			...
			await provider.addAssistaToStack(parentTask)
			await provider.addAssistaToStack(childTask)
		...
		test("correctly identifies subtask scenario for issue #4602", async () => {
			...
			expect(provider.getAssistaStackSize()).toBe(1)
	...
	test("add AssistaToStack adds multiple Assista instances to the stack", async () => {
		// Setup Assista instance with auto-mock from the top of the file
		const mockAssista1 = new Task(defaultTaskOptions) // Create a new mocked instance
		const mockAssista2 = new Task(defaultTaskOptions) // Create a new mocked instance
		Object.defineProperty(mockAssista1, "taskId", { value: "test-task-id-1", writable: true })
		Object.defineProperty(mockAssista2, "taskId", { value: "test-task-id-2", writable: true })

		// add Assista instances to the stack
		await provider.addAssistaToStack(mockAssista1)
		await provider.addAssistaToStack(mockAssista2)

		// verify assista instances were added to the stack
		expect(provider.getAssistaStackSize()).toBe(2)

		// verify current assista instance is the last one added
		expect(provider.getCurrentAssista()).toBe(mockAssista2)
	})
	...
	test("handles showAssistaIgnoredFiles setting", async () => {
		await provider.resolveWebviewView(mockWebviewView)
		const messageHandler = (mockWebviewView.webview.onDidReceiveMessage as any).mock.calls[0][0]

		// Default value should be true
		expect((await provider.getState()).showAssistaIgnoredFiles).toBe(true)

		// Test showAssistaIgnoredFiles with true
		await messageHandler({ type: "showAssistaIgnoredFiles", bool: true })
		expect(mockContext.globalState.update).toHaveBeenCalledWith("showAssistaIgnoredFiles", true)
		expect(mockPostMessage).toHaveBeenCalled()
		expect((await provider.getState()).showAssistaIgnoredFiles).toBe(true)

		// Test showAssistaIgnoredFiles with false
		await messageHandler({ type: "showAssistaIgnoredFiles", bool: false })
		expect(mockContext.globalState.update).toHaveBeenCalledWith("showAssistaIgnoredFiles", false)
		expect(mockPostMessage).toHaveBeenCalled()
		expect((await provider.getState()).showAssistaIgnoredFiles).toBe(false)
	})
	...
	describe("deleteMessage", () => {
		...
		test('handles "Just this message" deletion correctly', async () => {
			...
			const mockAssista = new Task(defaultTaskOptions) // Create a new mocked instance
			mockAssista.assistaMessages = mockMessages // Set test-specific messages
			mockAssista.apiConversationHistory = mockApiHistory // Set API history
			await provider.addAssistaToStack(mockAssista) // Add the mocked instance to the stack
			...
			expect(mockAssista.overwriteAssistaMessages).toHaveBeenCalledWith([
				mockMessages[0],
				mockMessages[1],
				mockMessages[4],
				mockMessages[5],
			...
			expect(mockAssista.overwriteApiConversationHistory).toHaveBeenCalledWith([
				mockApiHistory[0],
				mockApiHistory[1],
				mockApiHistory[4],
				mockApiHistory[5],
		...
		test('handles "This and all subsequent messages" deletion correctly', async () => {
			...
			// Setup Assista instance with auto-mock from the top of the file
			const mockAssista = new Task(defaultTaskOptions) // Create a new mocked instance
			mockAssista.assistaMessages = mockMessages
			mockAssista.apiConversationHistory = mockApiHistory
			await provider.addAssistaToStack(mockAssista)
			...
			expect(mockAssista.overwriteAssistaMessages).toHaveBeenCalledWith([mockMessages[0]])

			// Verify only API messages before the deleted message were kept
			expect(mockAssista.overwriteApiConversationHistory).toHaveBeenCalledWith([mockApiHistory[0]])
		})
		...
		test("handles Cancel correctly", async () => {
			...
			// Setup Assista instance with auto-mock from the top of the file
			const mockAssista = new Task(defaultTaskOptions) // Create a new mocked instance
			mockAssista.assistaMessages = [{ ts: 1000 }, { ts: 2000 }] as AssistaMessage[]
			mockAssista.apiConversationHistory = [{ ts: 1000 }, { ts: 2000 }] as (Anthropic.MessageParam & {
				ts?: number
			})[]
			await provider.addAssistaToStack(mockAssista)
			...
			expect(mockAssista.overwriteAssistaMessages).not.toHaveBeenCalled()
			expect(mockAssista.overwriteApiConversationHistory).not.toHaveBeenCalled()
		})
	})
...
describe("Project MCP Settings", () => {
	...
	test.skip("handles openProjectMcpSettings message", async () => {
		...
		expect(mockedFs.mkdir).toHaveBeenCalledWith("/test/workspace/.assista", { recursive: true })

		// Check that fs.writeFile was called with default content
		expect(mockedFs.writeFile).toHaveBeenCalledWith(
			"/test/workspace/.assista/mcp.json",
			JSON.stringify({ mcpServers: {} }, null, 2),
		)

		// Check that openFile was called
		expect(openFileSpy).toHaveBeenCalledWith("/test/workspace/.assista/mcp.json")
	})
	...
	test.skip("handles openProjectMcpSettings file creation error", async () => {
		...
		expect(vscode.window.showErrorMessage).toHaveBeenCalledWith(
			expect.stringContaining("Failed to create or open .assista/mcp.json"),
		)
	})
})
...
describe.skip("ContextProxy integration", () => {
	...
	beforeEach(() => {
		...
		provider = new AssistaProvider(mockContext, mockOutputChannel, "sidebar", mockContextProxy)
	})
...
describe("AssistaProvider - Router Models", () => {
	let provider: AssistaProvider
	let mockContext: vscode.ExtensionContext
	let mockOutputChannel: vscode.OutputChannel
	let mockWebviewView: vscode.WebviewView
