...
export async function applyDiffTool(
	...
	if (provider) {
		...
		if (!isMultiFileApplyDiffEnabled) {
			return applyDiffToolLegacy(assista, block, askApproval, handleError, pushToolResult, removeClosingTag)
		}
	}
	...
	if (block.partial) {
		...
		const sharedMessageProps: AssistaSayTool = {
			tool: "appliedDiff",
			path: getReadablePath(assista.cwd, filePath),
		}
...
Original error: ${errorMessage}`
		...
		const errorMsg = await assista.sayAndCreateMissingParamError(
			"apply_diff",
	...
	if (Object.keys(operationsMap).length === 0) {
		...
		assista.recordToolError("apply_diff")
		pushToolResult(
			await assista.sayAndCreateMissingParamError(
				"apply_diff",
				usingLegacyParams
	...
	try {
		...
		for (const operation of operations) {
			...
			if (!accessAllowed) {
				...
				updateOperationResult(relPath, {
					status: "blocked",
					error: formatResponse.assistaIgnoreError(relPath),
				})
				continue
			}
		...
		if (operationsToApprove.length > 1) {
			...
			const hasProtectedFiles = operationsToApprove.some(
				(opResult) => assista.assistaProtectedController?.isWriteProtected(opResult.path) || false,
			)

			// Prepare batch diff data
			const batchDiffs = operationsToApprove.map((opResult) => {
				const readablePath = getReadablePath(assista.cwd, opResult.path)
			...
			const { response, text, images } = await assista.ask("tool", completeMessage, hasProtectedFiles)
		...
		for (const opResult of operationResults) {
			...
			try {
				...
				const processedDiffItems = !assista.api.getModel().id.includes("claude")
					? diffItems.map((item) => ({
							...item,
				...
				const diffResult = (await assista.diffStrategy?.applyDiff(originalContent, processedDiffItems)) ?? {
					success: false,
...
${errorDetails ? `\nTechnical details:\n${errorDetails}\n` : ""}
				...
				if (successCount === 0) {
					if (formattedError) {
						const currentCount = assista.consecutiveMistakeCountForApplyDiff.get(relPath) || 0
						if (currentCount >= 2) {
				...
				await assista.diffViewProvider.update(originalContent!, true)
				...
				if (operationsToApprove.length === 1) {
					...
					if (assista.diffStrategy && assista.diffStrategy.getProgressStatus) {
						toolProgressStatus = assista.diffStrategy.getProgressStatus(
							{
								...block,
		...
		if (allDiffErrors.length > 0) {
			await assista.say("diff_error", allDiffErrors.join("\n"))
		}
