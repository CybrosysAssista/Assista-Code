...
type MockAssistaInstance = { taskId: string }
...
const mockInitAssistaWithTask = vi.fn<() => Promise<MockAssistaInstance>>().mockResolvedValue({ taskId: "mock-subtask-id" })
const mockEmit = vi.fn()
const mockRecordToolError = vi.fn()
const mockSayAndCreateMissingParamError = vi.fn()

// Mock the Assista instance and its methods/properties
const mockAssista = {
	ask: vi.fn(),
	...
	providerRef: {
		deref: vi.fn(() => ({
			...
			initAssistaWithTask: mockInitAssistaWithTask,
		})),
	},
}
...
describe("newTaskTool", () => {
	...
	it("should correctly un-escape \\\\@ to \\@ in the message passed to the new task", async () => {
		...
		await newTaskTool(
			mockAssista as any, // Use 'as any' for simplicity in mocking complex type
			block,
		...
		// Verify the message passed to initAssistaWithTask reflects the code's behavior in unit tests
		expect(mockInitAssistaWithTask).toHaveBeenCalledWith(
		...
		// Verify side effects
		expect(mockAssista.emit).toHaveBeenCalledWith("taskSpawned", expect.any(String)) // Assuming initAssista returns a mock task ID
		expect(mockAssista.isPaused).toBe(true)
		expect(mockAssista.emit).toHaveBeenCalledWith("taskPaused")
