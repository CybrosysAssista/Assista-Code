...
describe("read_file tool with maxReadFileLine setting", () => {
	...
	beforeEach(() => {
		...
		mockAssista.cwd = "/"
		mockAssista.task = "Test"
		mockAssista.providerRef = mockProvider
		mockAssista.assistaIgnoreController = {
			validateAccess: vi.fn().mockReturnValue(true),
		}
		mockAssista.say = vi.fn().mockResolvedValue(undefined)
		mockAssista.ask = vi.fn().mockResolvedValue({ response: "yesButtonClicked" })
		mockAssista.presentAssistantMessage = vi.fn()
		mockAssista.handleError = vi.fn().mockResolvedValue(undefined)
		mockAssista.pushToolResult = vi.fn()
		mockAssista.removeClosingTag = vi.fn((tag, content) => content)

		mockAssista.fileContextTracker = {
			trackFileContext: vi.fn().mockResolvedValue(undefined),
		}

		mockAssista.recordToolUsage = vi.fn().mockReturnValue(undefined)
		mockAssista.recordToolError = vi.fn().mockReturnValue(undefined)
	...
	describe("when maxReadFileLine is negative", () => {
		...
		it("should not show line snippet in approval message when maxReadFileLine is -1", async () => {
			...
			const askCall = mockAssista.ask.mock.calls[0]
...
describe("read_file tool XML output structure", () => {
	...
	beforeEach(() => {
		...
		mockAssista.ask = vi.fn().mockResolvedValue({ response: "yesButtonClicked" })
		mockAssista.presentAssistantMessage = vi.fn()
		mockAssista.sayAndCreateMissingParamError = vi.fn().mockResolvedValue("Missing required parameter")
		...
		mockAssista.recordToolUsage = vi.fn().mockReturnValue(undefined)
		mockAssista.recordToolError = vi.fn().mockReturnValue(undefined)
		mockAssista.didRejectTool = false
	...
	async function executeReadFileTool(
		...
		mockAssista.assistaIgnoreController.validateAccess = vi.fn().mockReturnValue(validateAccess)
	...
	describe("Error Handling Tests", () => {
		...
		it("should include error tag for AssistaIgnore error", async () => {
			...
			// Verify
			expect(result).toBe(
				`<files>\n<file><path>${testFilePath}</path><error>Access to ${testFilePath} is blocked by the .assistaignore file settings. You must try to continue in the task without using this file, or ask the user to update the .assistaignore file.</error></file>\n</files>`,
			)
		})
	})
})
