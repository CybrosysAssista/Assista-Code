...
vi.mock("../../prompts/responses", () => ({
	formatResponse: {
		...
		assistaIgnoreError: vi.fn((path) => `Access denied: ${path}`),
...
vi.mock("../../ignore/AssistaIgnoreController", () => ({
	AssistaIgnoreController: class {
		initialize() {
...
describe("writeToFileTool", () => {
	...
	beforeEach(() => {
		...
		mockAssista.cwd = "/"
		mockAssista.consecutiveMistakeCount = 0
		mockAssista.didEditFile = false
		mockAssista.diffStrategy = undefined
		mockAssista.assistaIgnoreController = {
			validateAccess: vi.fn().mockReturnValue(true),
		}
		mockAssista.diffViewProvider = {
			editType: undefined,
			isEditing: false,
			originalContent: "",
		...
		mockAssista.fileContextTracker = {
			trackFileContext: vi.fn().mockResolvedValue(undefined),
		}
		mockAssista.say = vi.fn().mockResolvedValue(undefined)
		mockAssista.ask = vi.fn().mockResolvedValue(undefined)
		mockAssista.recordToolError = vi.fn()
		mockAssista.sayAndCreateMissingParamError = vi.fn().mockResolvedValue("Missing param error")
	...
	async function executeWriteFileTool(
		...
		mockAssista.assistaIgnoreController.validateAccess.mockReturnValue(accessAllowed)
	...
	describe("access control", () => {
		it("validates and allows access when assistaIgnoreController permits", async () => {
			await executeWriteFileTool({}, { accessAllowed: true })

			expect(mockAssista.assistaIgnoreController.validateAccess).toHaveBeenCalledWith(testFilePath)
			expect(mockAssista.diffViewProvider.open).toHaveBeenCalledWith(testFilePath)
		})
	})
	...
	describe("file existence detection", () => {
		it.skipIf(process.platform === "win32")("detects existing file and sets editType to modify", async () => {
			...
			expect(mockAssista.diffViewProvider.editType).toBe("modify")
		})
		...
		it.skipIf(process.platform === "win32")("detects new file and sets editType to create", async () => {
			...
			expect(mockedFileExistsAtPath).toHaveBeenCalledWith(absoluteFilePath)
			expect(mockAssista.diffViewProvider.editType).toBe("create")
		})

		it("uses cached editType without filesystem check", async () => {
			mockAssista.diffViewProvider.editType = "modify"
	...
	describe("content preprocessing", () => {
		it("removes markdown code block markers from content", async () => {
			...
			expect(mockAssista.diffViewProvider.update).toHaveBeenCalledWith("Line 1\nLine 2", true)
		})

		it("passes through empty content unchanged", async () => {
			await executeWriteFileTool({ content: "" })

			expect(mockAssista.diffViewProvider.update).toHaveBeenCalledWith("", true)
		})

		it("unescapes HTML entities for non-Claude models", async () => {
			mockAssista.api.getModel.mockReturnValue({ id: "gpt-4" })
		...
		it("skips HTML unescaping for Claude models", async () => {
			mockAssista.api.getModel.mockReturnValue({ id: "claude-3" })
		...
		it("strips line numbers from numbered content", async () => {
			...
			expect(mockAssista.diffViewProvider.update).toHaveBeenCalledWith("line one\nline two", true)
		})
	})
	...
	describe("file operations", () => {
		it("successfully creates new files with full workflow", async () => {
			...
			expect(mockAssista.consecutiveMistakeCount).toBe(0)
			expect(mockAssista.diffViewProvider.open).toHaveBeenCalledWith(testFilePath)
			expect(mockAssista.diffViewProvider.update).toHaveBeenCalledWith(testContent, true)
			expect(mockAskApproval).toHaveBeenCalled()
			expect(mockAssista.diffViewProvider.saveChanges).toHaveBeenCalled()
			expect(mockAssista.fileContextTracker.trackFileContext).toHaveBeenCalledWith(testFilePath, "assista_edited")
			expect(mockAssista.didEditFile).toBe(true)
		})
	...
	describe("partial block handling", () => {
		it("returns early when path is missing in partial block", async () => {
			...
			expect(mockAssista.diffViewProvider.open).not.toHaveBeenCalled()
		})
		...
		it("streams content updates during partial execution", async () => {
			...
			expect(mockAssista.ask).toHaveBeenCalled()
			expect(mockAssista.diffViewProvider.open).toHaveBeenCalledWith(testFilePath)
			expect(mockAssista.diffViewProvider.update).toHaveBeenCalledWith(testContent, false)
		})
	})
	...
	describe("user interaction", () => {
		it("reverts changes when user rejects approval", async () => {
			...
			expect(mockAssista.diffViewProvider.revertChanges).toHaveBeenCalled()
			expect(mockAssista.diffViewProvider.saveChanges).not.toHaveBeenCalled()
		})

		it("reports user edits with diff feedback", async () => {
			const userEditsValue = "- old line\n+ new line"
			mockAssista.diffViewProvider.saveChanges.mockResolvedValue({
			...
			mockAssista.diffViewProvider.userEdits = userEditsValue

			await executeWriteFileTool({}, { fileExists: true })

			expect(mockAssista.say).toHaveBeenCalledWith(
				"user_feedback_diff",
	...
	describe("error handling", () => {
		it("handles general file operation errors", async () => {
			mockAssista.diffViewProvider.open.mockRejectedValue(new Error("General error"))

			await executeWriteFileTool({})

			expect(mockHandleError).toHaveBeenCalledWith("writing file", expect.any(Error))
			expect(mockAssista.diffViewProvider.reset).toHaveBeenCalled()
		})

		it("handles partial streaming errors", async () => {
			mockAssista.diffViewProvider.open.mockRejectedValue(new Error("Open failed"))
